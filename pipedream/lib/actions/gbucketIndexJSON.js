...
var gbucketIndexJSON = {
     ...
     */
        ...
        const key = JSON.parse(this.googleCloud.$auth.key_json);
        const encodeURISafe = (stringFragment) => encodeURIComponent(stringFragment)
            .replace(/'/g, "%27")
            .replace(/"/g, "%22");
        ...
        const { semvar, chainIdHex, prefix, bucket } = this.webhookTrigger
            .object;
        ...
        const routes = [];
        result[0]
            .filter((f) => f.name.indexOf("index.json") === -1 &&
            f.name.indexOf("defaults.json") === -1)
            .forEach((file) => {
            console.log(file.name);
            routes.push(file.name);
        });
        console.log(`⏳ Uploading index to Cloud Storage Bucket as https://firebasestorage.googleapis.com/v0/b/${bucket}/o/${encodeURISafe(filePath)}?alt=media \n`);
...
module.exports = gbucketIndexJSON;
//# sourceMappingURL=gbucketIndexJSON.js.map
